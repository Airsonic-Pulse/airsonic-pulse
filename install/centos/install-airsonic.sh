...
SYSCONFIG_FILE="/etc/sysconfig/airsonic"
...
JAVA_21_PKG="java-21-openjdk-headless"
JAVA_CURRENT_MAJOR=""
...
if [[ "${JAVA_CURRENT_MAJOR}" == "21" ]]; then
    ...
    case "${java_choice}" in
        1)
            dnf install -y "${JAVA_21_PKG}"
            JAVA_21_PATH="$(alternatives --list 2>/dev/null | grep java | grep 21 | awk '{print $3}' | head -1)"
            if [[ -n "${JAVA_21_PATH}" ]]; then
                alternatives --set java "${JAVA_21_PATH}"
                ok "Java 21 installed and set as default."
            else
                warn "Could not auto-configure alternatives for Java 21. Set it manually with: alternatives --config java"
            fi
            ;;
        2)
            dnf install -y "${JAVA_21_PKG}"
...
if [[ -f "${SYSCONFIG_FILE}" ]]; then
    ok "${SYSCONFIG_FILE} already exists — skipping."
else
    cat > "${SYSCONFIG_FILE}" << 'SYSCONFIG_EOF'
...
if ask "Install ffmpeg for transcoding support?"; then
    if ! dnf repolist enabled 2>/dev/null | grep -qi epel; then
        warn "EPEL repository not enabled. Enabling it now..."
...
if command -v firewall-cmd &>/dev/null && firewall-cmd --state &>/dev/null 2>&1; then
    if ask "Open port 4040 in firewalld?"; then
        firewall-cmd --permanent --add-port=4040/tcp
        firewall-cmd --reload
        ok "Port 4040 opened in firewalld."
    fi
else
