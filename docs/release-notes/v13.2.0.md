...
13.2.0 brings OpenSubsonic API support and API key authentication to Airsonic-Pulse. It implements the OpenSubsonic base compliance surface, the low-effort extensions (song lyrics, HTTP form POST, transcode offset), and API key authentication, along with a broad expansion of track-, album-, and artist-level metadata in API responses.

This release follows the 13.2.0 alpha, beta.1, and rc.1 previews. The notes below are cumulative against the last final release (v13.1.0): Upgrade notes and Summary of changes cover everything in 13.2.0.

## Upgrade notes

Everything operationally relevant to upgrading from v13.1.0.
...
* **podcastSettings 500 fixed (#296).** The podcast settings page threw an Internal Server Error on every load (a pre-existing immutable-list mutation, present since before 13.1.0). The page now loads.
...
* **logback-core 1.5.35 (CVE-2026-9828).** Upgraded logback, which includes the fix for CVE-2026-9828 (a deserialization whitelist bypass in `HardenedModelInputStream`) plus SSL and status-message hardening.
...
## New in this release since rc.1

* **logback-core upgraded to 1.5.35 (CVE-2026-9828).** A security upgrade of the logging library, including the fix for CVE-2026-9828 (a deserialization whitelist bypass) and additional SSL and status-message hardening. This was the only change between rc.1 and the final release.
...
### Documentation
* #306 Windows simple installer and launcher by @litebito in #307
...
### Infrastructure & CI
* ci: bump actions/stale from 10.2.0 to 10.3.0 by @app/dependabot in #220
* ci: bump actions/dependency-review-action from 4.9.0 to 5.0.0 by @app/dependabot in #221
* ci: bump github/codeql-action from 4.35.5 to 4.36.0 by @app/dependabot in #222
* ci: bump docker/build-push-action from 7.1.0 to 7.2.0 by @app/dependabot in #223
* ci: bump docker/login-action from 4.1.0 to 4.2.0 by @app/dependabot in #224
...
* deps: bump ch.qos.logback:logback-core from 1.5.25 to 1.5.35 in the maven group across 1 directory by @app/dependabot in #304

**Full Changelog**: https://github.com/Airsonic-Pulse/airsonic-pulse/compare/v13.1.0...v13.2.0
